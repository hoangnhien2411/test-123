...
async def receive_message_item(item: RTMessageItem, out_dir: str):
    ...
    async for contentPart in item:
        if contentPart.type == "audio":
            ...
            st.write(f'🤖 Bot: {audio_transcript}')
            #with open("transcript.txt", "a", encoding="utf-8") as f:
             #   f.write(str(audio_transcript))
...
async def receive_input_item(item: RTInputAudioItem):
    ...
    await item
    #print(f"User: {item.transcript}")
    file_path = "transcript.txt"
    st.write(f'🧑 User: {item.transcript}')
    with open(file_path, "a", encoding="utf-8") as f:
        f.write(str(item.transcript))
...
async def run(client: RTClient, audio_file_path: str, out_dir: str):
    ...
    # Đọc nội dung từ file transcript.txt
    ...
    # Kết hợp nội dung transcript vào instructions
    instructions = f"""
    ...
    await client.configure(
        ...
        instructions=instructions,  # Sử dụng hướng dẫn đã được kết hợp
        voice="alloy"
    )
    print("Done")
...
async def output_audio(filepath, outdir, provider='azure'):
    load_dotenv()
    print(filepath)
    file_path = filepath
    out_dir = outdir
    ...
    if provider == "azure":
        await with_azure_openai(file_path, out_dir)
